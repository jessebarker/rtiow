...
template<typename T>
const tvec2<T> operator*(const T t, const tvec2<T>& v)
{
    return v * t;
}

template<typename T>
const tvec3<T> operator*(const T t, const tvec3<T>& v)
{
    return v * t;
}

template<typename T>
const tvec4<T> operator*(const T t, const tvec4<T>& v)
{
    return v * t;
}

template<typename T>
tvec3<T> vmin(const tvec3<T>& v0, const tvec3<T>& v1)
{
    T x = std::min(v0.x(), v1.x());
    T y = std::min(v0.y(), v1.y());
    T z = std::min(v0.z(), v1.z());
    tvec3<T> result(x, y, z);
    return result;
}

template<typename T>
tvec3<T> vmax(const tvec3<T>& v0, const tvec3<T>& v1)
{
